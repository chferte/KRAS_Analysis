...
##############################################################
# assess the performance of the G12V_WT model in terms of AUC
###############################################################
...
require(ROCR)
for (i in c(1:N)){
  Pred <- prediction(as.numeric(yhat_BATTLE[,i]),as.numeric(G12V_BATTLE))
...
#plot(Perf, col="royalblue",main="predicting KRAS G12V in BATTLE 
...
#boxplot(yhat_CHEMORES~G12V_CHEMORES,xlab=c("KRAS G12V mutational status"),ylab="model of G12V",main="predicting KRAS G12V in CHEMORES
#(modele trained in TCGA)")
#stripchart(yhat_CHEMORES~G12V_CHEMORES,vertical=TRUE,pch=20,add=T,col="royalblue",cex=.7 )
...
AUC_CHEMORES <- c()
for (i in c(1:N)){
  Pred <- prediction(as.numeric(yhat_CHEMORES[,i]),as.numeric(G12V_CHEMORES))
  Perf <- performance(prediction.obj=Pred,"tpr","fpr")
  AUC <- performance(prediction.obj=Pred,"auc")
...
#boxplot(yhat_CCLE~G12V_CCLE,xlab=c("KRAS G12V mutational status"),ylab="model of G12V",main="predicting KRAS G12V in ccle
#(modele trained in TCGA)")
#stripchart(yhat_CCLE~G12V_CCLE,vertical=TRUE,pch=20,add=T,col="royalblue",cex=.7,method="jitter")
...
boxplot(AUC_CCLE)
# 
# plot(Perf, col="royalblue",main="predicting KRAS G12V in ccle
# (modele trained in TCGA)")
# text(x=.7,y=.4,labels=paste("AUC=",format(x=AUC@y.values,digits=2)),col="royalblue")
...
#boxplot(yhat_SANGER~G12V_SANGER,xlab=c("KRAS G12V mutational status"),ylab="model of G12V",main="predicting KRAS G12V in Sanger
#(modele trained in TCGA)")
#stripchart(yhat_SANGER~G12V_SANGER,vertical=TRUE,pch=20,add=T,col="royalblue",cex=.7, method="jitter")
...
boxplot(AUC_SANGER)
#
plot(Perf, col="royalblue",main="predicting KRAS G12V in Sanger
(modele trained in TCGA)")
text(x=.7,y=.4,labels=paste("AUC=",format(x=AUC@y.values,digits=2)),col="royalblue")
