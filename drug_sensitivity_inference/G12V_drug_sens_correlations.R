...
boxplot(G12V_SENS,cex.axis=.7,las=2, ylab="spearman correlation (r)",col=tmp)
abline(h=0,col="red",lty=2)
title("Correlation of 100 KRAS G12V bootstrapped gene expression models 
  with drug sensitivity in lung cancer cell lines (CCLE)",cex.main=.7)


G12V_SENS_CCLE_COR <- rbind(G12V_SENS)
...
if(length(which(sig.drugs==TRUE)) ==0){ print(NA) }else{
legend(-0.2,2,legend=names(G12V_SENS_CCLE_PVAL)[sig.drugs],
...
# boxplot(G12V_SENS[,1:15],col=tmp[1:15],cex.axis=.7,las=2,ylab="spearman correlation (r)")
# abline(h=0,col="red",lty=2)
# title("top 15 positive correlations G12V model with  
#   with drug sensitivity (SANGER)",cex.main=.7)

### plot the top 10 negative correlations
boxplot(G12V_SENS[,90:138],col=tmp[90:138],cex.axis=.7,las=2,ylab="spearman correlation (r)")
abline(h=0,col="red",lty=2)
title("top 15 negative correlations G12V model with  
  with drug sensitivity (SANGER)",cex.main=.7)


G12V_SENS_SANGER_COR <- rbind(G12V_SENS)
...
if(length(which(sig.drugs==TRUE)) ==0){ print(NA) }else{
  legend(-0.2,1,legend=names(G12V_SENS_SANGER_PVAL)[sig.drugs],
         pch=17,col=palette,cex=.8,xjust=.5)}
...
cex[sig.drugs] <- 1
plot(apply(G12V_SENS_CCLE_COR,2,mean)*-1, -log10(G12V_SENS_CCLE_PVAL),type="p",pch=pch,cex=cex,col=cols,
     ylab="-log10(p)",xlab="spearman r",xlim=c(-.5,.5))
abline(h=-log10(P),lty=2,col="gray60")
abline(v=c(-.4,.4),lty=2,col="gray60")
