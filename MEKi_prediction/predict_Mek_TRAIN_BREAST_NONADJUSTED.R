...
{
  par(mfrow=c(1,1))
  train <- sample(breast.mek.cells,replace=TRUE)
  val <-breast.mek.cells[-which(breast.mek.cells %in% train)]
...
#pen <- c(rep(1,times=dim(trainex)[1]-1),0)
  ...
  yhat.nsclc <- c(yhat.nsclc,list(predict(fit,t(global.matrix[,nsclc.mek.cells]))))
