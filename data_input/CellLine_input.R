...
getSangerDrug <- function(){
  pData(loadEntity("syn1445075")$objects$sangerIC50)
}
...
getSangerMut <- function(){
  tbl <- read.table("/home/jguinney/projects/h3/data~/sanger_cellline_mutation_data.txt",sep="\t",header=T, as.is=T, quote="",fill=T)
...
SangerInfo <- read.delim(file="~/FELLOW/cferte/KRAS_Analysis/data_input/Sanger_affy_n798_sample_info_published.txt",as.is=TRUE,header=TRUE )