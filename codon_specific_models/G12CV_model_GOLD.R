...
# for G12C +G12V phenotype ,  in TCGA LUAD
...
###############################################################
# run the analysis for the G12C in the TCGA LUAD  & CHEMORES (G12C & G12V vs any other KRAS)
###############################################################
...
#############################################################################
# focus on G12C G12V and WT only
#############################################################################

tmp <- names(KRAS_LUAD)[which(KRAS_LUAD %in% c("G12C","G12V","WT"))]
KRAS_LUAD <- KRAS_LUAD[tmp]
LUAD_EXP <- LUAD_EXP[,tmp]
rm(tmp)

tmp <- names(KRAS_CHEMORES)[which(KRAS_CHEMORES %in% c("G12C","G12V","WT"))]
KRAS_CHEMORES <- KRAS_CHEMORES[tmp]
CHEMORES_EXP <- CHEMORES_EXP[,tmp]
CHEMORES_CLIN <- CHEMORES_CLIN[tmp,]
rm(tmp)

tmp <- names(KRAS_CCLE)[which(KRAS_CCLE %in% c("G12C","G12V","WT"))]
KRAS_CCLE <- KRAS_CCLE[tmp]
CCLE_EXP <- CCLE_EXP[,tmp]
rm(tmp)

tmp <- names(KRAS_BATTLE)[which(KRAS_BATTLE %in% c("G12C","G12V","WT"))]
KRAS_BATTLE <- KRAS_BATTLE[tmp]
BATTLE_EXP <- BATTLE_EXP[,tmp]
rm(tmp)

tmp <- names(KRAS_SANGER)[which(KRAS_SANGER %in% c("G12C","G12V","WT"))]
...
tmp <- ifelse(KRAS_LUAD %in% c("G12C","G12V"),1,0)
...
par(mfrow=c(1,1))
require(glmnet)
G12CV_LUAD <- ifelse(KRAS_LUAD %in% c("G12C","G12V"),1,0)
table(G12CV_LUAD)
N <- 100
fit <- c()
#features <- c()
...
while(models<N)
{
  ...
  j <- c(names(which(G12CV_LUAD==1)), sample(names(which(G12CV_LUAD==0)),replace=TRUE))
  cv.fit <- cv.glmnet(t(LUAD_EXP[,j]), factor(G12CV_LUAD[j]), nfolds=3, alpha=.1, family="binomial")
  fit <- glmnet(x=t(LUAD_EXP[,j]),y=factor(G12CV_LUAD[j]),family="binomial",alpha=.1,lambda=cv.fit$lambda.1se)
